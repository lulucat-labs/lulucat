...
import { BinanceBridgeChainTask } from '@lulucat/tdk/defi';
...
const DEFAULT_FROM_CHAIN_ID = 42161; // Arbitrum
...
export default class BridgeTask extends AbsBase {
  ...
  public async execute(
    ...
    try {
      ...
      // 如果提币任务未完成，则跳过执行
      if (!tasks.exchangeToOkx) {
        console.log('提币任务未完成，跳过执行');
        throw WalletException.transactionFailed('提币未完成，跳过执行', {
          accountId: context.accountDetail.id,
          evmWallet: context.accountDetail.evmWallet,
        });
      }
      if (tasks.zKBridgeABS) {
        console.log('桥接任务已经完成，跳过执行');
        return {
          success: true,
          error: '桥接任务已经完成，跳过执行',
        };
      }
      const bridgeChainTask = new BinanceBridgeChainTask();
      ...
      const fromChainId = DEFAULT_FROM_CHAIN_ID;
      const toChainId = DEFAULT_TO_CHAIN_ID;
      ...
      // 设置最大可接受的Gas费用 (ETH)
      const maxFeeEth = config.maxFeeEth || FIXED_GAS_RESERVE_ETH;
      ...
      console.log(
        `从 ${this.getChainName(fromChainId)}(${fromChainId}) 桥接到 ${this.getChainName(toChainId)}(${toChainId})`,
      );
      ...
      throw WalletException.transactionFailed('桥接任务失败', {
        accountId: context.accountDetail.id,
        evmWallet: context.accountDetail.evmWallet,
      });
    }
