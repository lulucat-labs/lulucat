...
import { arbitrum, abstract } from 'viem/chains';
...
function initializeRelayClient(): void {
  // 转换链配置
  const arbitrumChain = convertViemChainToRelayChain(arbitrum);
  const abstractChain = convertViemChainToRelayChain(abstract);
  
  createClient({
    baseApiUrl: MAINNET_RELAY_API,
    chains: [arbitrumChain, abstractChain],
...
async function createWalletAndClient(privateKey: string, chainId: number) {
  ...
  // 获取RPC URL - 为Abstract使用更可靠的RPC节点
  let rpcUrl;
  let chain;
  ...
  if (chainId === 42161) { // Arbitrum
    ...
    chain = arbitrum;
  } else if (chainId === 2741) { // Abstract
    // 使用更可靠的Abstract RPC节点
    rpcUrl = 'https://rpc.abstrakt.network';
    chain = abstract;
  } else {
 ...
 * 使用 Relay SDK 进行资产跨链
 */
export class BinanceBridgeChainTask {
  /**
   * 执行桥接操作
    ...
    try {
      ...
      if (quoteDetails.fees.gasFeeEth > maxFeeEthBigInt) {
        ...
        throw WalletException.transactionFailed(`Gas 费用过高`, {
          walletInfo,
          message: `Gas 费用 ${formatEther(quoteDetails.fees.gasFeeEth)} ETH 超过设定的上限 ${maxFeeEth} ETH`
        });
      }
      ...
      if (relayerFeePercentage > maxRelayerFeePercentage) {
        ...
        throw WalletException.transactionFailed(`中继费占比过高`, {
          walletInfo,
          message: `中继费占比 ${relayerFeePercentage.toFixed(2)}% 超过阈值 ${maxRelayerFeePercentage}%`
        });
      }
